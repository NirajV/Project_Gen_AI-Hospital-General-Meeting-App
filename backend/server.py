...
async def register(user: UserCreate):
    ...
    await execute_query(
        ...
        (user_id, user.email, user.name, password_hash, user.specialty, user.organization, user.phone, user.role or 'doctor')
    )
    ...
    return {"access_token": token, "token_type": "bearer", "user": serialize_row(user_data)}
...
async def login(credentials: UserLogin):
    ...
    return {"access_token": token, "token_type": "bearer", "user": serialize_row(user)}
...
async def update_user(user_id: str, updates: dict, current_user: dict = Depends(get_current_user)):
    ...
    if set_parts:
        ...
        await execute_query(f"UPDATE users SET {', '.join(set_parts)} WHERE id = %s", tuple(values))
...
async def create_patient(patient: PatientCreate, current_user: dict = Depends(get_current_user)):
    ...
    await execute_query(
        ...
        (patient_id, patient.patient_id_number, patient.first_name, patient.last_name,
         patient.date_of_birth or None, patient.gender, patient.email, patient.phone, patient.address,
...
async def update_patient(patient_id: str, updates: dict, current_user: dict = Depends(get_current_user)):
    ...
    if set_parts:
        ...
        await execute_query(f"UPDATE patients SET {', '.join(set_parts)} WHERE id = %s", tuple(values))
...
async def create_meeting(meeting: MeetingCreate, current_user: dict = Depends(get_current_user)):
    ...
    await execute_query(
        ...
        (meeting_id, meeting.title, meeting.description, meeting.meeting_date,
         ...
         meeting.recurrence_end_date or None, current_user['id'])
    )
...
async def update_meeting(meeting_id: str, updates: dict, current_user: dict = Depends(get_current_user)):
    ...
    if set_parts:
        ...
        await execute_query(f"UPDATE meetings SET {', '.join(set_parts)} WHERE id = %s", tuple(values))
...
async def update_agenda_item(meeting_id: str, item_id: str, updates: dict, current_user: dict = Depends(get_current_user)):
    ...
    if set_parts:
        ...
        await execute_query(f"UPDATE agenda_items SET {', '.join(set_parts)} WHERE id = %s", tuple(values))
...
async def create_decision(meeting_id: str, decision: DecisionLogCreate, current_user: dict = Depends(get_current_user)):
    ...
    await execute_query(
        ...
        (decision_id, meeting_id, decision.meeting_patient_id, decision.agenda_item_id,
         ...
         decision.action_plan, decision.responsible_doctor_id, decision.follow_up_date or None,
...
async def update_decision(meeting_id: str, decision_id: str, updates: dict, current_user: dict = Depends(get_current_user)):
    ...
    if set_parts:
        ...
        await execute_query(f"UPDATE decision_logs SET {', '.join(set_parts)} WHERE id = %s", tuple(values))
...
@api_router.get("/")
async def root():
    return {"message": "Hospital Meeting Scheduler API", "status": "running", "database": "MySQL"}
...
async def health_check():
    try:
        ...
        return {"status": "unhealthy", "database": f"MySQL error: {str(e)}"}
...
async def startup():
    logger.info("Starting Hospital Meeting Scheduler API with MySQL")
    try:
        await get_db_pool()
