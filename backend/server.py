...
class TeamsLinkRequest(BaseModel):
    title: str
...
@api_router.post("/teams/generate-link")
async def generate_standalone_teams_link(
    payload: TeamsLinkRequest,
    current_user: dict = Depends(get_current_user),
):
    """Generate a Microsoft Teams meeting link without requiring an existing
    meeting record. Used by the meeting creation wizard so the link is
    available before the meeting is saved."""
    try:
        try:
            teams_service = get_teams_service()
        except ValueError as cfg_err:
            ...
            raise HTTPException(
                ...
                detail=(
                    "Microsoft Teams integration is not configured on this server. "
                    "Ask an administrator to set GRAPH_CLIENT_ID, GRAPH_TENANT_ID, "
                    "GRAPH_CLIENT_SECRET, and GRAPH_USER_ID in the backend "
                    "environment and restart the backend container."
                ),
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid date/time format. Expected meeting_date YYYY-MM-DD, start_time/end_time HH:MM.",
            )

        teams_meeting = await teams_service.create_online_meeting(
            subject=f"{payload.title} - Hospital Meeting",
            start_datetime=meeting_dt,
            end_datetime=end_dt,
        )

        logger.info(
            f"Standalone Teams link generated by user {current_user['id']}: "
            f"{teams_meeting.get('id')}"
        )
        ...
        logger.error(f"Failed to generate standalone Teams link: {str(e)}")
        raise HTTPException(
            status_code=502,
            detail=f"Failed to generate Teams meeting link: {str(e)}",
        )
